...
# Transform
voxelization_train	=	V2VVoxelization(cubic_size=cubic_size, original_size=100, augmentation=True)
voxelization_val	=	V2VVoxelization(cubic_size=cubic_size, original_size=100, augmentation=False)
...
if __name__ ==  '__main__':
	...
	# Dataset and loader
	train_set		=	ITOPDataset(data_dir, 'train',	transform_train)
	train_loader	=	torch.utils.data.DataLoader(train_set,	batch_size=batch_size, shuffle=True, num_workers=number_of_workers)

	# No separate validation dataset, just use test dataset instead
	val_set			=	ITOPDataset(data_dir, 'test',	transform_val)
	val_loader		=	torch.utils.data.DataLoader(val_set,	batch_size=batch_size, shuffle=False, num_workers=number_of_workers)
	...
	test_set = ITOPDataset(data_dir, 'test', transform_test)
	test_loader = torch.utils.data.DataLoader(test_set, batch_size=batch_size, shuffle=False, num_workers=number_of_workers)
	...
	fit_set = ITOPDataset(data_dir, 'train', transform_test)
	fit_loader = torch.utils.data.DataLoader(fit_set, batch_size=batch_size, shuffle=False, num_workers=number_of_workers)
	...
	test_set = ITOPDataset(data_dir, 'test')
	test_joints = []
	...
	fit_set = ITOPDataset(data_dir, 'train')	
	fit_joins = []
